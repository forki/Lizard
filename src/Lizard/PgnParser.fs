...
module PgnParser = 
    ...
    let pPiece = 
        (pchar 'p' >>% 0) <|> (pchar 'P' >>% 0) <|> (pchar 'N' >>% 2) <|> (pchar 'n' >>% 2) 
        <|> (pchar 'S' >>% 2) <|> (pchar 's' >>% 2) <|> (pchar 'B' >>% 1) <|> (pchar 'R' >>% 3) 
        <|> (pchar 'r' >>% 3) <|> (pchar 'Q' >>% 5) <|> (pchar 'q' >>% 5) <|> (pchar 'K' >>% 4) 
        <|> (pchar 'k' >>% 4) <?> "Piece (N, B, R, Q, K, P, n, r, q, k, p)"
    ...
    let pTarget = 
        ...
        <|> (pFile .>>. pRank |>> fun (f, r) -> ((0, f), r)) //Pawn move, e.g. d5
